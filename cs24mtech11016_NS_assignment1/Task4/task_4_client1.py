...
# READING p and g from environment variables
p = int(os.getenv("P", "23"))  # prime NO
...
local_ip = client_socket.getsockname()[0]    # Storing local ip i.e ip of client machine
local_port = client_socket.getsockname()[1]  # Storing local port i.e port of client machine when connecting to server
print(local_ip)
print(local_port)
...
def generate_private_key():               # to choose random number between 1 to p-1 as private key for this client
    return random.randint(1, p - 1)

def generate_public_key(private_key):     # calculating A to share with other client
    return pow(g, private_key, p)

def compute_shared_secret(public_key, private_key):  # calculating  same shared key 
    return pow(public_key, private_key, p)

def derive_key(shared_secret):
    return hashlib.sha256(str(shared_secret).encode()).digest()  # calculating AES key by  using same shared key as seed

def encrypt_message(message, encryption_key):
    cipher = AES.new(encryption_key, AES.MODE_CBC, iv=encryption_key[:16])
    ciphertext = cipher.encrypt(pad(message.encode(), AES.block_size))        #function to encrypt message before sending message using AES
    return cipher.iv + ciphertext  # Prepend IV to ciphertext

def decrypt_message(ciphertext, encryption_key):
    iv = ciphertext[:16]
    cipher = AES.new(encryption_key, AES.MODE_CBC, iv)                        #function to decrypt message after receiving message 
    ...
    while True:
        try:
            ...
            if message == "EOD":
                ...
                play_socket.close()                                               #function to send message whenever client want
                break
            else:
                print(f"[Plain_text]: {message}")
                encrypted_msg = encrypt_message(message, encryption_key)
...
def receive_client(play_socket, encryption_key):
    while True:
        try:
            ...
            if decrypted_msg == b"EOD":
                print(f"[Received Cipher_text]: {data}")                              #function to send receive whenever other client sent
    ...
    play_socket.close()
def r_peers():
    client_socket.sendall("SEND_PEERS".encode())                                  #function to send peer connected to servers
    data = client_socket.recv(BUFFER_SIZE).decode()
    print(data)
...
def l_fpeers():
    ...
    try:
        ...
        print("You are connected to peer 1")                                             #listening client will enter into this function as soon as client start to listen
...
def c_peer():
    ...
    try:
        ...
        print(f"You are connected to {ip_peer}")                                            #connecting  client will enter into this function as soon as client want  to connect with other peer
